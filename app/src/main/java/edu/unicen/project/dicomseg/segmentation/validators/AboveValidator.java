...
public class AboveValidator implements SegmentationValidator {

    private static List<String> errors = new ArrayList<String>();

    /**
     * Validates segmentation is above (y axis) its related segmentation
     *
     * @param points
     * @param toCompare
     * @param refX
            ...
            for (PointF polarPoint: segPolarPoints) {
                for (PointF polarPointToCompare: segToComparePolarPoints) {
                    if (polarPoint.x > polarPointToCompare.x) {
                        errors.add(SegmentationMessages.NOT_ABOVE_ERROR  + " " + toCompare.getType().getName());
